...
// CSS Imports
import '../styles/MobileExportPage.css';
...
const MobileExportPage = () => {
    ...
    const fetchSites = useCallback(async () => {
        try {
            ...
            setExportStatus({ success: false, message: 'Failed to load sites.' });
        }
    }, []);
    ...
    const fetchItems = useCallback(async () => {
        try {
            ...
            setExportStatus({ success: false, message: 'Failed to load items data.' });
        }
    }, []);

    const fetchAllTransactions = useCallback(async (uid) => {
        try {
            const q = query(collection(db, 'users', uid, 'transfers'));
            const querySnapshot = await getDocs(q);
            setAllTransactions(querySnapshot.docs.map((doc) => ({
                id: doc.id,
                ...doc.data(),
            ...
            setExportStatus({ success: false, message: 'Failed to load user transactions.' });
        }
    }, [db]);
    ...
    const fetchSignature = useCallback(async (uid) => {
        try {
            ...
            if (sigDoc.exists()) {
                setSignatureURL(sigDoc.data().url);
            } else {
    ...
    }, [location.search, sites, selectedSiteId]);
    ...
    const handleSignatureUpload = async (e) => {
        ...
        reader.onloadend = async () => {
            ...
            try {
                await setDoc(doc(db, 'users', userId, 'profile', 'signature'), {
                    url: base64String,
                });
                setSignatureURL(base64String);
            } catch (error) {
                console.error('Error uploading signature to Firestore:', error);
                if (error.code) {
    ...
    const calculateItemSiteTotals = useCallback((transactionsToProcess, targetSiteId = null, filterStartDate = null, filterEndDate = null) => {
        ...
        const filteredTxns = transactionsToProcess.filter(t => {
            ...
            if (filterStartDate) { const startOfDay = filterStartDate.toDate ? filterStartDate.toDate() : new Date(filterStartDate); startOfDay.setHours(0, 0, 0, 0); inDateRange = inDateRange && (txnDate >= startOfDay); }
            if (filterEndDate) { const endOfDay = filterEndDate.toDate ? filterEndDate.toDate() : new Date(filterEndDate); endOfDay.setHours(23, 59, 59, 999); inDateRange = inDateRange && (txnDate <= endOfDay); }
            return inDateRange;
        });

        filteredTxns.forEach(t => {
            if (targetSiteId && t.siteId !== targetSiteId) return;
            const siteId = t.siteId;
            const itemKey = `${t.itemId || 'unknown'}-${t.variantId || 'no-variant'}-${t.unit || 'no-unit'}`;
            if (!siteItemTotals[siteId]) siteItemTotals[siteId] = {};
            if (!siteItemTotals[siteId][itemKey]) {
                siteItemTotals[siteId][itemKey] = {
                    totalReceived: 0, totalUsed: 0, balance: 0,
                    itemName: t.itemName || 'Unknown Item', variantName: t.variantName || 'N/A',
                    unit: t.unit || 'pcs', itemId: t.itemId, variantId: t.variantId,
                };
            }
            ...
            if (t.type === 'inward') { siteItemTotals[siteId][itemKey].totalReceived += quantityInBase; }
            else if (t.type === 'outward') { siteItemTotals[siteId][itemKey].totalUsed += quantityInBase; }
        });
        ...
        for (const siteId in siteItemTotals) {
            ...
            for (const itemKey in siteData) {
                ...
                if (itemSummary.totalReceived === 0 && itemSummary.totalUsed === 0) continue;
                ...
                finalSummary.push({
                    siteId: siteId, siteName: siteName, itemName: itemSummary.itemName, variantName: itemSummary.variantName,
                    unit: preferredDisplayUnit,
                    totalReceived: parseFloat(itemSummary.totalReceived.toFixed(3)), // Use parseFloat
                    totalUsed: parseFloat(itemSummary.totalUsed.toFixed(3)), // Use parseFloat
                    balance: parseFloat(itemSummary.balance.toFixed(3)), // Use parseFloat
                    itemId: itemSummary.itemId, variantId: itemSummary.variantId,
                });
            }
        }
        return finalSummary;
    ...
    const displayedSummary = useMemo(() => {
        ...
        const currentSiteTotals = calculateItemSiteTotals(allTransactions, selectedSiteId, exportStartDate, exportEndDate);
        return currentSiteTotals;
    }, [allTransactions, sites, selectedSiteId, exportStartDate, exportEndDate, calculateItemSiteTotals]);

    const currentSiteName = useMemo(() => {
        const site = sites.find(s => s.id === selectedSiteId);
        return site ? site.name : 'Selected Site';
    ...
    const saveFile = async (blob, fileName) => {
        try {
            ...
            console.log(`Capacitor: File saved to app's private Documents directory: ${Directory.Documents}/${filePath}`);

            const fileUriResult = await Filesystem.getUri({ directory: Directory.Documents, path: filePath });
    ...
    const handleExportWrapper = async (exporterFunction) => {
        ...
        try {
            ...
            setExportStatus({
                success: true,
                message: `File "${fileNameResult}" exported successfully! Check your device's Files/Documents app or use the share dialog to move it.`
            });
        } catch (error) {
    ...
    const generatePDF = async () => {
        ...
        // Convert Day.js objects to native Date objects for toLocaleDateString()
        ...
        const groupedItems = displayedSummary.reduce((acc, item) => {
            ...
            const convertedReceived = convertUnit(item.totalReceived, item.unit, baseUnitForCalculation);
            const convertedUsed = convertUnit(item.totalUsed, item.unit, baseUnitForCalculation);
            const convertedBalance = convertUnit(item.balance, item.unit, baseUnitForCalculation);
        ...
        docPdf.autoTable({
            ...
            didParseCell: function(data) {
                if (data.section === 'body' && data.column.index === 1 && data.cell.raw && typeof data.cell.raw === 'string' && data.cell.raw.startsWith('  ')) {
        ...
        allUniqueItemIdsInSelectedSite.forEach(uniqueItemId => {
            ...
            const itemTransactions = transactionsForExportDetailSheets.filter(t =>
                t.itemId === uniqueItemId && t.siteId === selectedSiteId
            ...
            if (itemDetails && itemTransactions.length > 0) {
                ...
                const summaryForItem = displayedSummary.find(s => s.itemId === itemDetails.id && s.siteId === selectedSiteId);
    ...
    const shareOnWhatsApp = async () => {
        if (!selectedSiteId || displayedSummary.length === 0) {
        ...
        try {
            ...
            setExportStatus({
                success: true,
                message: `PDF generated and saved to **mobile's Documents folder**! A share dialog has opened. Please select WhatsApp.`
            });
    ...
    const openExportedFolder = async () => {
        if (isCapacitorApp()) {
            try {
                ...
                setExportStatus({ success: true, message: "Instructions provided to locate files. Please use your device's file manager." });
            } catch (error) {
                console.error("Error attempting to open file manager:", error);
                setExportStatus({ success: false, message: "Could not open file manager directly. Please navigate to app's Documents folder manually." });
            }
        } else {
            alert("This feature is for mobile devices. On web, files are saved in your browser's default downloads folder.");
        }
    };
    ...
    if (loading) {
        return (
            <Container maxWidth="md" sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '80vh' }}>
                <CircularProgress />
                <Typography variant="h6" sx={{ ml: 2 }}>Loading data for export...</Typography>
            </Container>
        );
    }
    ...
    return (
        <Container maxWidth="lg" className="mobile-export-page-container">
                        ...
                        <CardContent>
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                                ...
                                <Button
                                    ...
                                    disabled={uploading}
                                >
                                    {uploading ? <CircularProgress size={24} /> : 'Upload Signature'}
                                    <input type="file" hidden accept="image/png, image/jpeg" onChange={handleSignatureUpload} />
                                </Button>
                            </Box>
                            {uploading && <Typography variant="body2" color="text.secondary">Uploading signature...</Typography>}
                        </CardContent>
                    </Card>
                </Grid>
                <Grid item xs={12}>
                    <Card variant="outlined">
                        <CardContent>
                            ...
                            <FormControl fullWidth variant="outlined" sx={{ mb: 2 }}>
                                <InputLabel id="site-select-label">Select Site for Report</InputLabel>
                                <Select
                                    labelId="site-select-label"
                                    value={selectedSiteId}
                                    onChange={(e) => setSelectedSiteId(e.target.value)}
                                    label="Select Site for Report"
                                    disabled={loading || exporting || sites.length === 0}
                                >
                                    {sites.length === 0 ? (
            ...
            {exportStatus.message && (
                <Box
                    sx={{
                        mt: 2, p: 1.5, borderRadius: 1, textAlign: 'center', fontWeight: 'bold',
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, alignItems: 'center' }}>
                <Button
                    ...
                    disabled={displayedSummary.length === 0 || !selectedSiteId || exporting || !storagePermission}
                    sx={{ py: 1.5 }}
                >
                    {exporting ? <CircularProgress size={24} color="inherit" /> : 'Generate & Share PDF Report'}
                </Button>
                <Button
                    variant="contained"
                    fullWidth={isMobile}
                    ...
                    sx={{ py: 1.5 }}
                >
                    {exporting ? <CircularProgress size={24} color="inherit" /> : 'Generate & Share Excel Report'}
                </Button>
                <Button
                    variant="outlined"
                    fullWidth={isMobile}
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mt: 3, textAlign: 'center' }}>
                **Note:** Reports are saved to your app's Documents folder (e.g., `Internal Storage / Android / data / YOUR_APP_ID / files / Documents`). Use your device's File Manager to access them, or the share dialog to save elsewhere.
            </Typography>
        </Container>
    );
};

export default MobileExportPage;